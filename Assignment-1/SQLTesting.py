...
print("Reinitializing database using populate-se.sql")

# The next steps will drop and recreate the whole 'stackexchange' database and populate it using populate-se.sql.
# We assume the environment is such that this process is permitted and you have sufficient privileges.

os.system("dropdb --if-exists stackexchange")
os.system("createdb stackexchange")
os.system("psql -d stackexchange -U root -f populate-se.sql")

# Reconnect after re-initialization
...
### Test queries to run -- there is a fair amount of specialized logic here to test the various queries. We would recommend going through the code to understand what it is doing.
